...
export default function NewListingView() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim()) nextErrors.title = "Property title is required.";
    ...
    if (rent === "") {
      ...
      nextErrors.rent = "No negative allowed.";
    }

    if (!availabilityDate) {
      nextErrors.availabilityDate = "Availability date is required.";
    } else if (availabilityDate < today) {
      nextErrors.availabilityDate = "Availability date cannot be in the past.";
    }

    if (!location.trim()) nextErrors.location = "Location address is required.";
    if (!city.trim()) nextErrors.city = "City is required.";

    if (photos.length === 0) {
      nextErrors.photos = "Please upload at least one photo of the property.";
    }
          ...
          <div className="glass-panel p-8 rounded-2xl border border-[#eae1d3] bg-white shadow-2xl space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-5 text-xs">
              <div>
                ...
                />
                {fieldErrors.title && (
                  <p className="mt-1.5 text-[11px] text-red-600">{fieldErrors.title}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {fieldErrors.rent && (
                    <p className="mt-1.5 text-[11px] text-red-600">{fieldErrors.rent}</p>
                  )}
                </div>
                <div>
              ...
              <div className="space-y-3 pt-2">
                ...
                {fieldErrors.photos && (
                  <p className="text-[11px] text-red-600">{fieldErrors.photos}</p>
                )}
