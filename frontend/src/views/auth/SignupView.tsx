      ...
      <div className="flex min-h-screen bg-white font-sans overflow-hidden">
        ...
        <div className="w-full lg:w-2/5 flex flex-col justify-center px-8 sm:px-16 xl:px-24 py-12 relative z-20 bg-white shadow-[-20px_0_40px_rgba(44,39,36,0.05)] h-screen overflow-y-auto custom-scrollbar">
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-sm mx-auto my-auto">
            ...
            <div className="mb-8">
              <h2 className="text-3xl font-serif font-bold text-[#2c2724] mb-2">{step === "OTP" ? "Verify Email" : "Create Account"}</h2>
              <p className="text-sm font-medium text-[#8a7d6a]">{step === "OTP" ? `Enter the OTP sent to ${email}` : "Join our verified housing network today."}</p>
            </div>
            ...
            <AnimatePresence mode="wait">
              {step === "FORM" && (<motion.div key="signup-form" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                ...
                <form onSubmit={handleSignup} className="space-y-4">
                  ...
                  <button type="submit" disabled={loading || !allPasswordRulesPassed} className="w-full bg-[#cfa052] hover:bg-[#b58942] disabled:opacity-50 disabled:cursor-not-allowed text-white py-3.5 rounded-2xl font-bold text-sm shadow-[0_8px_20px_rgba(207,160,82,0.3)] transition-all flex justify-center items-center gap-2 group mt-4 cursor-pointer">{loading ? "Creating Account..." : "Create Account"} {!loading && <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />}</button>
                </form>
